<x-admin-layout>
    ...
    <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
        <main class="w-full flex-grow p-6">
            <h1 class="w-full text-3xl text-black pb-6">Banners</h1>

            <div class="w-full mt-12">
                <p class="text-xl pb-3 flex items-center">
                    <i class="fas fa-list mr-3"></i> Banner Records
                </p>
                @can('create', 'App\Models\Banner')
                    <button class="px-4 py-1 text-white font-light tracking-wider bg-blue-600 rounded mb-2"
                        onclick="location.href='{{ route('admin.banner.create') }}';">Add Banner</button>
                @endcan
                        ...
                        <tbody>
                            @foreach ($banners as $banner)
                                <tr class="hover:bg-grey-lighter">
                                    ...
                                    <td class="py-4 px-6 border-b border-grey-light">{{ $banner->title }}</td>       
                                    <td class="py-4 px-6 border-b border-grey-light">
                                        @can('update', $banner)
                                            <button
                                                class="px-4 py-1 text-white font-light tracking-wider bg-green-600 rounded"
                                                type="button"
                                                onclick="location.href='{{ route('admin.banner.edit', $banner->id) }}';">Edit</button>
                                        @endcan
                                        @can('delete', $banner)
